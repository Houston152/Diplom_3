...
class TestBaseFunctional:
    @allure.title('Переход пользователя в "Конструктор"')
    def test_switch_to_constructor(self, driver, login):
        ...
        assert main_page.check_build_a_burger_text(), 'Переход в "Конструктор" не выполнен'

    @allure.title('Переход пользователя в "Лента заказов"')
    def test_switch_to_order_feed(self, driver, login):
        main_page = MainPage(driver)
        main_page.click_button_order_feed()
        order_feed = OrderFeedPage(driver)
        assert order_feed.check_order_feed_text(), 'Переход в "Лента заказов" не выполнен'
    ...
    def test_ingredient_details(self, driver, login):
        ...
        assert main_page.check_modal_window_details_ingredient(), 'Всплывающее окно "Детали ингредиента" не появилось'

    @allure.title('Закрытие всплывающего окна "Детали ингредиента" кликом по крестику')
    def test_close_modal_window_details_ingredient(self, driver, login):
        main_page = MainPage(driver)
        main_page.click_ingredient()
        main_page.click_button_close_in_modal_window()
        assert main_page.check_modal_window_details_ingredient(), 'Всплывающее окно "Детали ингредиента" не закрыто'
    ...
    def test_counter_ingredients(self, driver, login):
        ...
        assert int(counter_1) < int(counter_2), 'Счётчик ингредиента, добавленного в заказ, не увеличился'
    ...
    def test_successful_create_order(self, driver, login):
        ...
        assert main_page.check_order_is_processed(), 'Заказ у авторизованного пользователя не оформлен'
