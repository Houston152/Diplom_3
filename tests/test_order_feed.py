...
class TestOrderFeedPage:
    ...
    def test_counter_orders_of_all_time_increases(self, driver, login):
        ...
        assert int(counter_1) < int(counter_2), \
            'Счётчик "Выполнено за всё время" при создании нового заказа не увеличивается'
    ...
    def test_counter_orders_for_today_increases(self, driver, login):
        ...
        assert int(counter_1) < int(counter_2), \
            'Счётчик "Выполнено за сегодня" при создании нового заказа не увеличивается'
    ...
    def test_number_order_appears_in_work(self, driver, login):
        ...
        assert str(order_number) == order_number_in_work[1:], 'Номер нового заказа в разделе "В работе" не появляется'
